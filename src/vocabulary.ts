 ...
 */
export function isBackgroundAgentsNotice(source: MessageSource): boolean {
  const candidate = source as { kind?: string; plugin?: string; form?: string }
  if (candidate.form !== 'notice') return false
  if (candidate.kind === SOURCE_KIND) return true
  return candidate.kind === 'plugin' && candidate.plugin === PLUGIN
}
