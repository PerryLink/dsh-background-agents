...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-background-agents?metric=downloads&lang=hi)](https://dshfind.com/hi/plugins/PerryLink/dsh-background-agents?ref=badge)
...
0.1.2-rc.1 (2026-09-04 को अनुकूलित): सत्र लिफ़ाफ़ा अपना ignorable फ़ील्ड केवल संग्रहीत-लॉग पठन संगतता के लिए रखता है - Session.append अभी भी इसे स्टैम्प नहीं कर सकता (तीसरा पैरामीटर केवल सतह इवेंट प्रकारों के लिए SurfaceIntent है, कभी विकल्प-बैग नहीं), इसलिए तथ्य-गेट व्यवहार अपरिवर्तित है।
0.1.3-alpha.1 (2026-09-06 को अनुकूलित): CI हार्नेस पिन मास्टर checkout (`d347e7039`) पर चला जाता है - session-persistence सेवा का हैंडल सीम (`open → read → close`)। प्रकाशित 0.1.2-rc.1 रनटाइम open() से पहले का है, इसलिए bg_result की कोल्ड रीड सीम का पता लगाकर load() पर फ़ॉलबैक करती है - दोनों लाइनों पर व्यवहार समान है। 2026-09-06 को dsh-v0.1.7-alpha.1 मास्टर checkout के विरुद्ध सत्यापित (पूर्ण गेट श्रृंखला + प्रोफ़ाइल इंस्टॉल स्मोक)।
0.1.5-alpha.1 (2026-09-09 को अनुकूलित): `SessionHandleReadResult` अब `{ eventState, events }` लौटाता है, इसलिए bg_result की कोल्ड रीड हैंडल सीम से `.events` डीस्ट्रक्चर करती है (प्रकाशित लाइन का `load()` फ़ॉलबैक अपरिवर्तित है)। CI हार्नेस पिन सार्वजनिक tag commit `5dda764ed3aa` पर चला जाता है (स्थानीय checkout 13 infra commits आगे है और CI से पहुँच से बाहर) और compat जाँचें alpha लाइन इंस्टॉल करती हैं। peers को `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0` तक बढ़ाया गया (prerelease-tuple नियम: अकेला पहला ब्राज़ `0.1.5-alpha.1` से मेल नहीं खाता) और devDeps `0.1.5-alpha.1` पर पिन हैं। 2026-09-09 को `dsh-v0.1.7-alpha.1` के विरुद्ध सत्यापित (पूर्ण गेट श्रृंखला)। पहले का `0.1.3-alpha.1` दावा अब अप्रचलित है: वह प्रीरिलीज़ दोनों peer ब्राज़ों से बाहर है।
...
0.1.7-alpha.1 (2026-09-22 को अनुकूलित): होस्ट ने उत्पादक-स्वामित्व वाली संदेश एट्रिब्यूशन पर अपना स्थानांतरण पूरा किया — कैच-ऑल `{ kind: 'plugin', plugin }` स्रोत अब टाइप मैप और भौतिक-पंक्ति प्रवेश, दोनों जगह से हटा दिया गया है — इसलिए यह प्लगइन जो भी नोटिस इंजेक्ट करता है, अब वह अपना `kind: 'dsh-background-agents'` लेकर चलता है (मॉड्यूल ऑगमेंटेशन द्वारा घोषित); प्रोजेक्शन पुराने लॉग से `'plugin'` वाले नोटिस अब भी पढ़ता है। टूल परिणाम अब V4 प्रथम-श्रेणी के `role: 'tool'` संदेश हैं (`toolCallId`/`content`/`isError` शीर्ष स्तर पर, कोई `{ type: 'tool-result' }` आवरण ब्लॉक नहीं), और `listChildren` केवल-पहचान वाली `SubagentCatalogEntry` पंक्तियों पर लौट आया है — कोई `kind` विभेदक नहीं, कोई डायग्नोस्टिक नहीं, कोई `activity` नहीं — इसलिए प्रत्येक प्रत्यक्ष-लिस्टिंग कॉल साइट `mode === 'continuable'` से वर्गीकृत करती है। क्लाइंट की ओर, `IconBranchOutline16` अब `IconBranchOutlineRegular` है और `ISessions.refreshSubagents` अब `refreshProjections(sessionId)` है। 2026-09-22 को सत्यापित (दोनों typecheck रूलर + पूरी टेस्ट सूट + build + आर्टिफ़ैक्ट जाँच); ब्राउज़र-दृश्य आधा भाग अभी **मशीन-सत्यापित नहीं** है।

| सतह | स्थिति |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.7-alpha.2` (GitHub tag, 2026-09-22 को सत्यापित; dev पिन `0.1.7-alpha.2`, peers `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0`) |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
- **इंजेक्ट किए गए `user/message` नोटिस** (मॉडल-दृश्य), स्रोत `{ kind: 'dsh-background-agents', form: 'notice' }` — थ्रॉटल्ड प्रगति पंक्तियाँ और संग्रह नोटिस (कैनोनिकल उपसर्ग `[background-agent <id>] …`)।
