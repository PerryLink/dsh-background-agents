...
function registerBackgroundAgentTools(ctx, config, lifecycle) {
	ctx.tools.register(defineTool({
		...
		description: "Start a background agent: a durable child agent session that keeps working while this conversation continues. It receives the task as its first message, runs it in its own context, and returns a stable agent id immediately. Track it with bg_list, watch its progress lines appear in this conversation (autoReport), send it more work any time with bg_message, and request a stop with bg_stop. Progress summaries are injected into this conversation after each of its turns and its final outcome arrives as a notice when it settles. Use this for long-running or parallel objectives you want to steer over time.",
		parameters: {
			task: {
				type: "string",
				required: true,
		...
		async execute(_args, exec) {
			...
			for (const entry of entries) {
				...
				const row = {
					...
					label: entry.label,
					mode: "continuable",
					activity: fact === void 0 ? activityOf(ctx, entry.id, fallbackFact(entry.id)) : activityOf(ctx, entry.id, fact)
				};
				if (fact !== void 0) {
...
const Config = Schema.object({
	...
	reportThrottleMs: Schema.natural().max(Number.MAX_SAFE_INTEGER).default(15e3),
	...
	idleSweepIntervalMs: Schema.natural().max(Number.MAX_SAFE_INTEGER).default(6e4),
...
function apply(ctx, config) {
	const policy = {
		...
		reportThrottleMs: config.reportThrottleMs ?? 15e3,
		...
		idleSweepIntervalMs: config.idleSweepIntervalMs ?? 6e4,
