...
*/
function isBackgroundAgentsNotice(source) {
	const candidate = source;
	if (candidate.form !== "notice") return false;
	if (candidate.kind === "dsh-background-agents") return true;
	return candidate.kind === "plugin" && candidate.plugin === "dsh-background-agents";
}
...
*/
function sessionLastText(events, options = {}) {
...
*/
function asRegistryStateSchema$1(schema) {
	return schema;
}
/** Bridge one zod-v3 schema into the registry's `wire.viewSchema` slot (see above). */
function asRegistryViewSchema$1(schema) {
	return schema;
}
...
const backgroundAgentsProjectionDefinition = {
	...
	stateSchema: asRegistryStateSchema$1(import_zod.z.object({ entries: import_zod.z.array(backgroundAgentEntrySchema.extend({ source: import_zod.z.enum(["legacy", "event"]) }).strict()) }).strict()),
	init: () => ({ entries: [] }),
	apply(state, event) {
		switch (event.type) {
			case FACT_EVENT: {
	...
	wire: {
		viewSchema: asRegistryViewSchema$1(backgroundAgentsSchema),
...
*/
async function deliverQueuedPrompt(subagents, parent, childId, content, source, signal) {
	const runtime = subagents;
	const masterQueue = runtime[Symbol.for("dsh.subagent.deliverPrompt")];
	if (typeof masterQueue === "function") return await masterQueue.call(runtime, parent, childId, content, source, signal, "queue");
	const alphaQueue = runtime[Symbol.for("dsh.subagent.queuePrompt")];
	...
	if (typeof followup !== "function") throw new Error("bg_message delivery unavailable: this harness line exposes neither the queue symbols nor subagents.followup");
	return await followup.call(subagents, parent, childId, content, {
		source,
		signal
	});
}
...
function registerBackgroundAgentTools(ctx, config, lifecycle, facts) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const messageId = await deliverQueuedPrompt(ctx.subagents, parent, childId, [{
				type: "text",
				text: message
			}], {
				kind: "agent-message",
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (!known) try {
				known = (await ctx.subagents.listChildren(parent.id, exec.signal)).some((entry) => entry.mode === "continuable" && entry.id === childId);
			} catch (error) {
			...
			if (live !== void 0 && typeof live.snapshotEvents === "function") events = live.snapshotEvents();
			else {
				...
				if (persistence !== void 0) {
					if (typeof persistence.open === "function") {
						...
						await handle.close();
					} else events = (await persistence.load(childId)).events;
				}
			}
			const reasoning = { used: false };
			const text = events === void 0 ? "unavailable (no live session and no durable log read face on this host)" : sessionLastText(events, {
				allowReasoning: true,
				reasoning
			});
...
* schemas with zod v3 (`dependencies.zod: ^3.24.0`); the two majors are not
* structurally assignable. The durable read boundary only calls `parse` on the
* schema, which both majors implement identically — the cast is a deliberate,
* documented boundary bridge, not a hidden mismatch.
...
var RoomHub = class extends Service {
	...
	*/
	hasMember(sessionId) {
		...
		if (rooms === void 0) return false;
		for (const [, record] of rooms.entries()) if (this.memberOf(record, sessionId) !== void 0) return true;
		return false;
	}
...
*/
async function catchUpWithTimeout(hub, sessionId, signal) {
	let handle;
	const timeout = new Promise((_, reject) => {
		handle = setTimeout(() => {
			reject(/* @__PURE__ */ new Error(`room catch-up exceeded ${CATCH_UP_TIMEOUT_MS} ms`));
		}, CATCH_UP_TIMEOUT_MS);
	});
	const cancelled = new Promise((_, reject) => {
		if (signal === void 0) return;
		if (signal.aborted) {
			reject(/* @__PURE__ */ new Error("room catch-up cancelled"));
			return;
		}
