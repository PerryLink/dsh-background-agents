...
* options bag, so fact events land unmarked and stricter hosts refuse to
* resume those sessions (`SessionFormatUnsupportedError`). The fact
* appender detects the host before polluting a log: the installed peer
* version is checked against the known-unmarked lines first, and an
...
* silently drop the marker from `Session.append` options. Non-matching
* (post-rc.6, stable, or unresolvable) versions are treated as
...
var FactAppender = class {
	...
	support = "unknown";
	warned = false;
	constructor(allowUnmarked, warn) {
...
function apply(ctx, config) {
	...
	const facts = new FactAppender(config.allowUnmarkedFacts ?? DEFAULTS.allowUnmarkedFacts, (message) => ctx.logger("background-agents").warn(message));
	if (ctx.get("storageDomain") === void 0) ctx.logger("background-agents").info("team rooms disabled: no storage domain composed (add @deepseek-ai/dsh-storage-domain to enable the /room command and the room_* tools)");
