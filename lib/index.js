...
function reportProgress(agents, sessions, config, lifecycle, child, now) {
	...
	parent.session.append(FACT_EVENT, {
		kind: "progress",
...
function archiveChild(ctx, agents, config, lifecycle, child) {
	...
	if (parent !== void 0) parent.session.append(FACT_EVENT, {
		kind: "archived",
		agentId: child.childId
	}, { ignorable: true });
...
var RoomHub = class extends Service {
	...
	readyResolve = () => {};
	initError;
	constructor(ctx, config, agents, sessions) {
		super(ctx, "roomHub");
		this.config = config;
		this.agents = agents;
		...
		return this.onMemberChain(sessionId, async () => {
			...
			for (const [, record] of rooms.entries()) {
				...
				for (const event of await this.timelineOf(record.roomId, member.lastFactSeq)) {
					...
					if (fact !== void 0) session.append(TEAM_ROOM_FACT, fact, { ignorable: true });
				}
