...
function childLastText(sessions, childId) {
	...
	return sessionLastText(typeof session.snapshotEvents === "function" ? session.snapshotEvents() : session.events);
}
...
function registerBackgroundAgentTools(ctx, config, lifecycle, facts) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (live !== void 0) events = typeof live.snapshotEvents === "function" ? live.snapshotEvents() : live.events;
			else {
			...
			const text = events === void 0 ? "" : sessionLastText(events, {
				allowReasoning: true,
				reasoning
			});
