...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-background-agents?metric=downloads&lang=es)](https://dshfind.com/es/plugins/PerryLink/dsh-background-agents?ref=badge)
...
0.1.3-alpha.1 (adaptado el 2026-09-06): el pin de CI del harness pasa al checkout master (`d347e7039`) - el seam de handles (`open → read → close`) del servicio session-persistence. El runtime publicado 0.1.2-rc.1 es anterior a open(), así que la lectura fría de bg_result detecta el seam y recurre a load() - mismo comportamiento en ambas líneas. Verificado el 2026-09-06 contra el checkout master dsh-v0.1.7-alpha.1 (cadena completa de gates + smoke de instalación de perfil).
0.1.5-alpha.1 (adaptado el 2026-09-09): `SessionHandleReadResult` ahora devuelve `{ eventState, events }`, así que la lectura fría de bg_result desestructura `.events` del seam de handle (el fallback `load()` de la línea publicada no cambia). El pin de CI del harness pasa al commit público del tag `5dda764ed3aa` (el checkout local está 13 commits de infra por delante e inalcanzable desde CI) y las sondas de compatibilidad instalan la línea alpha. Los peers se amplían a `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0` (regla prerelease-tuple: el primer brazo por sí solo no coincide con `0.1.5-alpha.1`) y los devDeps fijan `0.1.5-alpha.1`. Verificado el 2026-09-09 contra `dsh-v0.1.7-alpha.1` (cadena completa de gates). La afirmación anterior de `0.1.3-alpha.1` queda obsoleta: esa versión preliminar cae fuera de ambos brazos de peers.
...
0.1.7-alpha.1 (adaptado el 2026-09-22): el host completó su migración a la atribución de mensajes propiedad del productor —la fuente catch-all `{ kind: 'plugin', plugin }` se retira tanto del mapa de tipos como de la admisión de filas físicas—, así que cada aviso que inyecta este plugin lleva ahora su propio `kind: 'dsh-background-agents'`, declarado por aumento de módulo; la proyección sigue leyendo avisos con `'plugin'` de logs antiguos. Los resultados de herramientas son mensajes V4 de primera clase con `role: 'tool'` (`toolCallId`/`content`/`isError` en el nivel superior, sin bloque envoltorio `{ type: 'tool-result' }`), y `listChildren` volvió a filas `SubagentCatalogEntry` de solo identidad —sin discriminante `kind`, sin diagnósticos y sin `activity`—, así que cada punto de llamada de listado directo clasifica por `mode === 'continuable'`. En el cliente, `IconBranchOutline16` pasó a `IconBranchOutlineRegular` y `ISessions.refreshSubagents` a `refreshProjections(sessionId)`. Verificado el 2026-09-22 (ambas reglas de typecheck + la suite completa + build + comprobaciones de artefactos); la mitad visible en el navegador **no** está verificada en máquina todavía.

| Superficie | Estado |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.7-alpha.2` (tag de GitHub, verificado el 2026-09-22; pines de desarrollo `0.1.7-alpha.2`, peers `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0`) |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
- **avisos `user/message` inyectados** (visibles para el modelo), fuente `{ kind: 'dsh-background-agents', form: 'notice' }` — las líneas de progreso con límite de frecuencia y los avisos de archivado (prefijo canónico `[background-agent <id>] …`).
