...
- `dsh.manifestVersion: 1` and the canonical three-clause `engines.dsh`.
- **Adapt to the `0.1.7-alpha.1` host line.** The dev/test pin moves to the published `0.1.7-alpha.1` packages. This is a deliberate deviation from the "next line" reading of the upgrade brief, which suggested `0.1.5-rc.3`: that line still carries the *old* contracts (`SettingsProvider`, `installSection`, `kind: 'plugin'`, the old `JobSnapshot`), so the seam fixes below could not typecheck against it. Four independent host changes land here:
  - **Producer-owned message attribution replaces the retired `kind: 'plugin'` catch-all.** `MessageSourceMap` now carries only `user | model | tool | 'system-prompt'` plus each producer's own kind, and the persistence layer (`session-format-v3-to-v4`) refuses a physical row whose source `kind` is `'plugin'` — so `as any` cannot smuggle one past admission either. This package declares `'dsh-background-agents'` by module augmentation and stamps it on every notice it injects (`src/vocabulary.ts`; the declaration sits in the one module both writers import). The projection still **reads** historical `'plugin'`-kinded notices, because it folds arbitrary older logs and the notice-line prefix remains the authority.
  - **Tool results are V4 first-class `role: 'tool'` messages.** The `{ type: 'tool-result' }` content block no longer exists in any form: the message carries a top-level `toolCallId` and `content` (plus optional `isError`), and the persistence layer refuses a nested wrapper. Read sites in the dev demo and the projection spec were migrated to read the role and its top-level fields; the replay-metadata channel (`tool/result.meta`) is unchanged.
  - **`SubagentCatalogEntry` is identity-only now.** `listChildren` no longer returns the `SubagentListEntry` union: a direct-child listing reads the parent-owned `subagentCatalog` projection and no child log, so its rows carry no `kind` discriminator, no diagnostics, and no `activity`. Every direct-listing call site now classifies by `mode === 'continuable'` and the non-recursive `bg_list` branch no longer fabricates a diagnostic row. Diagnostics stay on `listDescendants`, which really does inspect every child log.
  - **Client contract drift.** `IconBranchOutline16` became `IconBranchOutlineRegular` (the 0.1.7 icon set names glyph and weight separately and takes `size` as a prop), and `ISessions.refreshSubagents` was replaced by `refreshProjections(sessionId)`. `ISessions` retains `subagentAddress`, so the CS1 behavior above is unchanged.

### Docs

- **Correction.** The `[Unreleased]` entry crediting the `agent/session-start` → `agent/created` rename to the "0.1.6-alpha.1 checkout" is wrong about the version. `git grep "agent/session-start"` returns **zero** hits at both `dsh-v0.1.6-alpha.1` and `dsh-v0.1.7-alpha.1`, and `agent/created` was already in wide use at the 0.1.6-alpha.1 baseline (`packages/context/file-reference-local/src/index.ts:92`). The rename therefore predates that window and the original entry misattributes it. The historical line above is left as published; only this note is new. No behavior changes with it.
...
- `pnpm-workspace.yaml` carries an `overrides` block for the 0.1.7 line. Three rows are real floors, not redundancy: `cordis` 4.0.3 (4.0.2 does not export `Volatile`), `cosmokit` 1.8.4 (its re-export source), and `schemastery` 3.18.3 (the copy the alpha.1 host packages were built against) — the nominal `^4.0.2`/`^3.18.2` ranges cover them, but an existing lockfile keeps the older patch. The remaining rows are self-referential pins: pnpm matches a prerelease range only against its own `[major, minor, patch]` tuple, so without them a transitive `^0.1.7-alpha.1` peer resolves separately and installs a second copy of the host type graph. Peer ranges are deliberately left **unchanged** (no supported host line is dropped).
- Two packages named in the upgrade brief are **not** dependencies of this repo and needed no rename: `@deepseek-ai/dsh-settings-file` and `@deepseek-ai/dsh-agent-presets`. The brief's hit counts resolve to generated artifacts only — the `dsh-agent-presets` hits live in `pnpm-lock.yaml` as a peer edge of the *published* `@deepseek-ai/dsh-subagent@0.1.6-alpha.2` (gone once the pin moves to alpha.1, whose peer set names `dsh-agent-preset-registry`), and both names appear in `tsconfig.paths.json` / `vitest.aliases.mjs` purely because `scripts/gen-aliases.mjs` mirrors the host checkout's `packages/**/package.json` inventory. Neither name occurs in `src`, `tests`, `dev`, or `package.json`. This package never read `ctx.settings`/`ctx.settingsScope` or the service key `agentPresets`.
